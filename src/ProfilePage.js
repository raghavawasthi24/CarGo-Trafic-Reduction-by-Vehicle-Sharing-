...
const ProfilePage = () => {
  ...
  useEffect(() => {
    if (requested == true) {
      axios
        .get("https://divyanshurana312.pythonanywhere.com/vehicle/allrides/4/")
        .then((res) => {
    ...
    <div className="profile-page">
      ...
      <div className="ridesPage">
        <div className="show-all-rides">
          <div
            onClick={ridesRequested}
            className={turn ? "requestedRide" : "nothing"}
          >
            Rides Requested
          </div>
          <div
            onClick={ridesOffered}
            className={turn ? "nothing" : "requestedRide"}
          >
            Rides Offered
          </div>
        </div>
